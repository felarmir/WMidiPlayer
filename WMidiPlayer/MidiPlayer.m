...
- (void)play
{
	...
	if (ms == nil || released || !(stopped || paused)) {
		...
		MusicPlayerStart(mp);
        //MusicPlayerSetPlayRateScalar(mp, 10);
		stopped = NO;
		paused = NO;
	}
...
- (BOOL)setupPlayer:(NSString*)path withInstruments:(NSArray*)programs {
	...
	NewMusicSequence(&ms);
    NSURL * midiFileURL = [NSURL URLWithString:path];
...
- (void)setupGraph:(NSArray*)programs {
	...
	NSString *bankPath = [[NSBundle mainBundle] pathForResource:@"FantaGM32" ofType:@"sf2"];
